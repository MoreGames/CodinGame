...
public:
	...
	Move() : position(0), next(0), target(0), stepsToTarget(0), pods(0) {
	}
	Move(int p, int n, int t, int s, int po) : position(p), next(n), target(t), stepsToTarget(s), pods(po) {
	}
};
...
private:
	...
	vector<int> moveTargets(Zone* zones, int id) {
		...
		if (attackMode) {
			targets.push_back(targetBase);
		}
		else {
			for (int i = 0; i < zoneCount; i++) {
				Zone z = zones[i];
				if (z.platinum > 0 && z.owner != id) {
					int numberOfPods = 0;
					for (int j = 0; j < 4; j++) {
						if (j == id) continue;
	...
	vector<pair<int, int>> availablePods(Zone* zones, int id) {
		vector<pair<int, int>> pods(0);
		for (int i = 0; i < zoneCount && pods.size()<350; i++) {
			Zone z = zones[i];
			if (z.pods[id] > 0) {
				if (!z.isFighting) { // pods from fighting zones never move
					if (z.platinum == 0) { // pods from zones with no resources always move
						if (groupMode) {
							pods.push_back(make_pair(z.id, z.pods[id]));
						}
						else {
							for (int j = 0; j < z.pods[id]; j++) {
								pods.push_back(make_pair(z.id, 1));
							}
						}
					}
					else {
						...
						if (m > z.platinum) {
							...
							if (groupMode && p > 0) {
								pods.push_back(make_pair(z.id, p));
							}
							else {
								for (int j = 0; j < p; j++) {
									pods.push_back(make_pair(z.id, 1));
	...
	vector<vector<int>> weightPodsTargets(Zone* zones, vector<pair<int, int>>& pods, vector<int>& targets) {
		...
		for (int i = 0; i < rows; i++) {
			...
			while (j < mcols) {
				for (int n = 0; n < cols && j < mcols; n++, j++) {
					weights[i][j] = (int)(zones[targets[n]].value / distanceToAll[pods[i].first][targets[n]] * 100);
				}
			}
	...
	vector<vector<int>> bestPodTargetCombination(Zone* zones, vector<pair<int, int>>& pods, vector<int>& targets) {
		...
		fprintf(stderr, "pods: %d targets: %d dim: %d\n", pods.size(), targets.size(), max(weights.size(), weights[0].size()));
		hungarian.solve();
	...
	vector<Move> computeMoves(int playerId, Zone* zones, vector<Move> enemyMoves) {
		...
		if (targets.size() > 0) {
			vector<pair<int, int>> pods = availablePods(zones, playerId);
			...
			if (pods.size() > 0) {
				...
				for (size_t i = 0; i < pods.size(); i++) {
					...
					start = &zonesOrigin[pods[i].first];
					...
					if (path.size() > 1) {
						...
						moves.push_back(Move(start->id, next->id, target->id, path.size() - 1, pods[i].second));
...
public:
	...
	int startZone(int id) {
		for (int i = 0; i < zoneCount; i++) {
			if (zonesOrigin[i].pods[id] > 0)
				return i;
		}
		return 0;
	}
	void analyse(int homeId, int enemyId) {
		Zone *homeBase, *enemyBase;

		homeBase = &zonesOrigin[homeId];
		enemyBase = &zonesOrigin[enemyId];
		...
		unordered_map<int, Zone*> came_from = dijkstra(zonesOrigin, homeBase, enemyBase, costZone, 0);
		vector<Zone*> path = reconstructPath(homeBase, enemyBase, came_from);

		int distToEnemyBase = path.size() - 1;

		fprintf(stderr, "distToEnemyBase: %d \n", distToEnemyBase);

		if (distToEnemyBase < 6) {
			groupMode = true;
			attackMode = true;
		}
...
public:
	...
	Player() : platinum(0), platinumNext(0), income(0), allPlatinum(0), numberOfPods(0), numberOfZones(0), startZone(0) {
	}
	...
	void status() {
		fprintf(stderr, "%4d %8d %6d %7d %9d %10d  %2d \n", platinum, platinumNext, income, allPlatinum, numberOfPods, numberOfZones, startZone);
	}
...
private:
	...
	void status() {
		if (!turn) {
			...
			for (int i = 0; i < playerCount; i++) {
				players[i].startZone = world->startZone(i);
				if (i != myId) {
					targetBase = players[i].startZone;
				}
			}
		}

		fprintf(stderr, "player overview: \n");
		fprintf(stderr, "player plat platNext income allPlat numOfPods numOfZones  sZ \n");
		for (int i = 0; i < playerCount; i++) {
	...
	void analysing() {
		world->analyse(players[0].startZone, players[1].startZone);
	}
	void moving() {
	...
	void buying() {
		int numberOfReinforcements = 0;//players[myId].numberOfPods>50 ? 0 : (players[myId].platinum / 20);
...
public:
	...
	void play() {
		while (1) {
			...
			if (interval > 0.1) groupMode = true;
			fprintf(stderr, "clock cycles: %f seconds elapsed: %f groupMode: %d \n", endTime, interval, groupMode);

...
public:
	...
	int shortestPath(int from, vector<int> targets) {
		...
		//cout << "shortest path from " << from << " to target(s), next cell: " << cell << " (distance: " << dist << ")" << endl;
	...
	vector<Move> wallMoves() {
		...
		for (int j = 0; j < players.size(); j++) {
			if (j != myId) {
				vector<int> walls = wallPositionsArea(players[j].position, 1);
...
public:
	...
	void printGraph() {
		cout << "graph: " << endl;
		fprintf(stdout, "x y ix |  0  1  2  3\n");
		fprintf(stdout, "--------------------\n");
		for (int i = 0; i < R * C; i++) {
			fprintf(stdout, "%d %d %2d | %2d %2d %2d %2d\n", i%C, i / R, i, graph[i * 4 + 0], graph[i * 4 + 1], graph[i * 4 + 2], graph[i * 4 + 3]);
		}
	}
	void printWallPositions() {
		...
		for (int i = 0; i < (R - 1); i++) {
			cout << i << " | ";
			for (int j = 0; j < (C - 1); j++) {
				fprintf(stdout, "%d ", wallPositions[i * (C - 1) + j]);
			}
			cout << endl;
		}
		fprintf(stdout, "-------------------\n");
		for (int j = 0; j < (C - 1); j++) {
			cout << j << " | ";
			for (int i = 0; i < (R - 1); i++) {
				fprintf(stdout, "%d ", wallPositions[i * (C - 1) + j + (R - 1) * (C - 1)]);
			}
			cout << endl;
		}
		cout << "wall positions left: " << l << endl;
		fprintf(stdout, "    ");
		for (int i = 0; i < C; i++) {
			fprintf(stdout, "%d ", i);
		}
	...
	vector<Move> allMoves(int playerId) {
		...
		//if (players[playerId].wallsLeft > 0) {
		//	vector<Move> moves;
...
*/
		...
		return (81. - dist[playerId]) / 81.*0.75 - ((81. - dist[(playerId+1)%2]) / 81.*0.05) + 0.20*players[playerId].wallsLeft / 10.;
	}
	...
	string nextMove() {
		...
		for (auto move : moves) {
			cout << move << " " << convertNumberToWallMove(move) << endl;
		}
...
public:
	...
	void loadInitData() {
		...
		FILE* initFile = fopen("Tests\\E2Start.in", "r");
		...
		if (initFile == NULL) {
			...
			fscanf(initFile, "%d %d %d %d", &w, &h, &playerCount, &myId);
	...
	void loadRoundData(Board& board) {
		...
		FILE* startFile = NULL;
		if (turn == 0) {
			startFile = fopen("Tests\\E2Round1.in", "r");
		} else if (turn == 1) {
			startFile = fopen("Tests\\E2Round2.in", "r");
		} else if (turn == 2) {
			startFile = fopen("Tests\\E2Round3.in", "r");
		} else if (turn == 3) {
			startFile = fopen("Tests\\E2Round4.in", "r");
		} else if (turn == 4) {
			startFile = fopen("Tests\\E2Round5.in", "r");
		} else if (turn == 5) {
			startFile = fopen("Tests\\E2Round6.in", "r");
		} else if (turn == 6) {
			startFile = fopen("Tests\\E2Round7.in", "r");
		} else if (turn == 7) {
			startFile = fopen("Tests\\E2Round8.in", "r");
		} else if (turn == 8) {
			startFile = fopen("Tests\\E2Round9.in", "r");
		} else if (turn == 9) {
			startFile = fopen("Tests\\E2Round10.in", "r");
		}
		...
		if (startFile == NULL) {
			...
			for (int i = 0; i < playerCount; i++) {
				...
				int y;
				int wallsLeft;
				fscanf(startFile, "%d %d %d", &x, &y, &wallsLeft);
			...
			int wallCount;
			fscanf(startFile, "%d", &wallCount);

			for (int i = 0; i < wallCount; i++) {
				int wallX, wallY;
				char wallOrientation; // wall orientation ('H' or 'V')
				fscanf(startFile, "%d %d %c", &wallX, &wallY, &wallOrientation);
...
*/
	...
	void play() {
		while (1) {
			...
			fprintf(stdout, "action clock cycles: %f seconds elapsed: %f \n", endTime, interval);
